...
namespace Test
{
    public  class OpenCvDetectBarcodes
    {
        ...
        private static List<DetectedObject> DetectBarcodesByOpenCv(Stream imageStream)
        {
            ...
            {
                // 先判断文件是否存在，不存在直接抛清晰异常
                if (!File.Exists(detect_prototxt))
                    throw new FileNotFoundException("找不到 detect.prototxt", detect_prototxt);
                if (!File.Exists(detect_caffemodel))
                    throw new FileNotFoundException("找不到 detect.caffemodel", detect_caffemodel);
                if (!File.Exists(sr_prototxt))
                    throw new FileNotFoundException("找不到 sr.prototxt", sr_prototxt);
                if (!File.Exists(sr_caffemodel))
                    throw new FileNotFoundException("找不到 sr.caffemodel", sr_caffemodel);
            ...
            for (int retry = 0; retry < maxRetries; retry++)
            {
                try
                {
                    ...
                    using (var image = Cv2.ImDecode(imageBytes, ImreadModes.Color))
                    {
                        ...
                        {
                            Console.WriteLine($"⚠️ 警告：图片读取失败或文件已损坏，已自动跳过！");
                        ...
                        using (var processedImage = new Mat())
                        {
                            ...
                            {
                                // 第4次：切块识别，直接救回最后那张！
                                bool success = SplitAndDetect(image, weChatQRCode, detectedObjects);
                                if (success)
                                {
                                    break; // 识别到了，直接退出
                                }

                                break;  //未识别到,也退出，没必要继续识别
                            }
                            ...
                            {
                                //执行解析，没结果就开始重试
                                bool flowControl = DetectAndDecode(detectedObjects, processedImage);
                                if (!flowControl)
                                {
                                    continue;
                                }
                            ...
                            catch (Exception ex)
                            {
                              ...
                              retry < maxRetries - 1)
                                ...
                                {
                                    Console.WriteLine($"❌ 识别底层异常：{ex.Message}，图片路径");
                                    break;
                                }
                            }
                            finally
        ...
        //传入需要识别的文件
        private static bool DetectAndDecode(List<DetectedObject> detectedObjects, Mat processedImage)
        {
            Mat[] rects = null;
            string[] texts = null;
            try
            {
                ...
                {
                    Console.WriteLine($"ℹ️ 未识别到二维码内容，图片路径");
                    return false;
                }
            }
            catch (Exception)
        ...
        private static bool SplitAndDetect(Mat src, WeChatQRCode qr, List<DetectedObject> results)
        {
            int w = src.Width;
            int h = src.Height;
            ...
            foreach (var rect in regions)
            {
                try
                {
                    ...
                    bool flowControl = DetectAndDecode(results, block);
                    if (!flowControl)
                    {
                        continue;
                    }
                    ...
                    //        DetectedObject detectedObject = new DetectedObject();
                    //        detectedObject.Type = "QR_CODE";
                    //        if (!string.IsNullOrEmpty(t) && !results.Any(s => s.Content != null && s.Content.Contains(t)))
                    //        {
        ...
        // 安全释放rects数组的方法
        private static void SafeDisposeRects(Mat[] rects)
        {
            if (rects != null)
            {
